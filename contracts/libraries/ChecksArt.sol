...
library ChecksArt {
    ...
    function getCheck(
        ...
        check.seed = (uint256(
            keccak256(abi.encodePacked(randomness, stored.seed))
        ) % type(uint128).max);
    ...
    function colorBandIndex(
        ...
        return
            divisorIndex == 0
                ? (
                    n > 80 ? 0 : n > 40 ? 1 : n > 20 ? 2 : n > 10 ? 3 : n > 4
                        ? 4
                        : n > 1
                        ? 5
                        : 6
    ...
    function colorIndexes(
        ...
        if (check.hasManyChecks) {
            if (gradient > 0) {
                ...
                for (uint256 i = 1; i < checksCount; ) {
                    indexes[i] =
                        (indexes[0] +
                            (((i * gradient * colorBand) / checksCount) %
                                colorBand)) %
                        80;
                    unchecked {
                        ++i;
                ...
                for (uint256 i = 1; i < checksCount; ) {
                    indexes[i] =
                        (indexes[0] + Utilities.random(seed + i, colorBand)) %
                        80;
                    unchecked {
                        ++i;
                    }
        ...
        if (divisorIndex > 0) {
            ...
            uint256[] memory parentIndexes = colorIndexes(
                previousDivisor,
                check,
                checks
            );
            ...
            uint256[] memory compositedIndexes = colorIndexes(
                previousDivisor,
                composited,
                checks
            );
    ...
    function colors(
        ...
        uint256[] memory indexes = colorIndexes(
            check.stored.divisorIndex,
            check,
            checks
        );
    ...
    function perRow(uint8 checks) public pure returns (uint8) {
        return
            checks == 80 ? 8 : checks >= 20 ? 4 : checks == 10 || checks == 4
                ? 2
                : 1;
    }
    ...
    function rowX(uint8 checks) public pure returns (uint16) {
        return
            checks <= 1 ? 286 : checks == 5 ? 304 : checks == 10 || checks == 4
                ? 268
                : 196;
    }
    ...
    function generateChecks(
        ...
        for (uint8 i; i < checksCount; i++) {
            ...
            string memory color = data.check.isRevealed
                ? data.colors[i]
                : data.colors[0];
    ...
    function collectRenderData(
        ...
        (string[] memory colors_, uint256[] memory colorIndexes_) = colors(
            check,
            checks
        );
