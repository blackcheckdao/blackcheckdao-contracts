...
contract Checks is IChecks, CHECKS721 {
    ...
    function getCheck(
        uint256 tokenId
    ) external view returns (Check memory check) {
    ...
    function simulateComposite(
        uint256 tokenId,
        uint256 burnId
    ) public view returns (Check memory check) {
        ...
        if (index < 5) {
            (uint8 gradient, uint8 colorBand) = _compositeGenes(
                tokenId,
                burnId
            );
        ...
        check.composite = !check.isRoot && index < 7
            ? check.stored.composites[index]
            : 0;
    ...
    function simulateCompositeSVG(
        ...
        return
            string(
                ChecksArt.generateSVG(
                    simulateComposite(tokenId, burnId),
                    checks
                )
            );
    }
    ...
    function svg(uint256 tokenId) external view returns (string memory) {
        return
            string(
                ChecksArt.generateSVG(
                    ChecksArt.getCheck(tokenId, checks),
                    checks
                )
            );
    }
    ...
    function _composite(uint256 tokenId, uint256 burnId) internal {
        (StoredCheck storage toKeep, , uint8 divisorIndex) = _tokenOperation(
            tokenId,
            burnId
        );
        ...
        emit Composite(
            tokenId,
            burnId,
            ChecksArt.DIVISORS()[toKeep.divisorIndex]
        );
        emit MetadataUpdate(tokenId);
    }
    ...
    /// @param tokenId The token ID to keep.
    /// @param burnId The token ID to burn.
    function _compositeGenes(
        uint256 tokenId,
        uint256 burnId
    ) internal view returns (uint8 gradient, uint8 colorBand) {
        ...
        uint256 randomizer = uint256(
            keccak256(abi.encodePacked(keeper.seed, burner.seed))
        );
